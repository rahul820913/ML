  ...
  const handleSubmit = async () => {
    ...
    try {
      if (!token) {
        ...
        alert("You must be logged in to submit the timetable.");
        return;
      }

      await axios.post(
        "http://localhost:3001/api/users/create/timetable",
        timetableData,
        {
      ...
      console.error("Error adding timetable:", error.response?.data || error);
      alert(`Failed to add timetable: ${error.response?.data?.message || "Unknown error"}`);
    }
  };
        ...
        {schedule.map((item, index) => (
          <li key={index} className="schedule-item">
            ...
            <div className="schedule-buttons">
              <button className="btn btn-edit" onClick={() => editScheduleEntry(index)}>✏️ Edit</button>
